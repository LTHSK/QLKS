...
public class OrderDAO {
    ...
    public ArrayList<Order> getAlLOrderWithStatus() {
        ...
        ArrayList<Order> list = new ArrayList<>();
        try (Connection conn = DatabaseConnection.opConnection(); PreparedStatement pstmt = conn.prepareStatement("SELECT *  FROM [QLKS].[dbo].[Order] where status = N'Chưa thanh toán'")) {
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    String ma = rs.getString("orderID");
                    String checkOutDate = rs.getString("checkOutDate");
